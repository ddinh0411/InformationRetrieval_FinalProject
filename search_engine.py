...
class FinancialIR:
    ...
    def _load_initial_corpus(self, tokenized_doc_path, norm_doc_path):
        with open(tokenized_doc_path, 'r', encoding='utf-8') as f:
            tokenized_data = json.load(f)
        with open(norm_doc_path, 'r', encoding='utf-8') as f:
    ...
    def evaluate_IR(self, queries_list, qrels_dict):
        ...
        for query in queries_list:
            ...
            results = self.search(query['text'], n=len(self.doc_ids))
    ...
    def update_corpus(self, new_docs):
        ...
        self.bm25 = BM25Okapi(self.corpus_tokens)
        print(f"Update Complete. Total documents now: {len(self.doc_ids)}")
